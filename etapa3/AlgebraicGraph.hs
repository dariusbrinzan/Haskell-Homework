...
extend f graph = case graph of
                    Node x -> 
                        f x
                    Overlay g1 g2 -> 
                        Overlay (extend f g1) (extend f g2)
                    Connect g1 g2 -> 
                        Connect (extend f g1) (extend f g2) 
...
helper :: Eq a
        ...
        -> [a]
        -> AlgebraicGraph a
helper x node targets = if null targets then
                           Empty
                        else 
                           Overlay (Node $ head targets) (helper x node $ tail targets)
splitNode :: Eq a
          => a                 -- nodul divizat
          -> [a]               -- nodurile cu care este înlocuit
          -> AlgebraicGraph a  -- graful existent
          -> AlgebraicGraph a  -- graful obținut
splitNode node targets = (\x -> (extend $ \y -> if y == node then
                                                  helper y node targets
                                               else 
                                                  Node y          
                                        ) x)
