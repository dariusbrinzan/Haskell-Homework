...
removeNode node graph = fromComponents [x | x <- S.toList $ nodes graph, x /= node]
                                       [(x, y) | (x, y) <- S.toList $ edges graph, x /= node && y /= node]
{-
    *** TODO ***
...
splitNode old news graph = let
                            outN = outNeighbors old graph
                            inN = inNeighbors old graph
                            new_node = (foldr (\x result -> (x : result)) $ S.toList $ nodes $ removeNode old graph) news
                            list = ([(x, y) | (x, y) <- S.toList $ edges graph, x /= old, y /= old] ++
                                    [(x, y) | x <- news, y <- S.toList outN] ++
                                    [(x, y) | y <- news, x <- S.toList inN])
                            in
                                fromComponents new_node list
{-
    *** TODO ***
...
mergeNodes prop node graph = 
                            let
                                old = [x | x <- S.toList $ nodes graph, not $ prop x]
                                len = length $ S.toList $ nodes graph
                                n_node = if len /= length old then old ++ [node] else old
                                n_edge = [x | x <- S.toList $ edges (foldr (\x result -> if not $ prop x then
                                                                                              result
                                                                                            else
                                                                                              splitNode x [node] result) graph (nodes graph))]
                            in 
                                fromComponents n_node n_edge
